...
class Student extends MY_Controller {
    ...
    * ********************************************************** */
    ...
    public function get_single_student(){
        ...
        $this->data['invoices'] = $this->student->get_invoice_list($student->school_id, $student->user_id); 
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $item_issues = $this->student->get_list('item_issues' ,  array('user_id' => $student->user_id, 'return_date'=>NULL), '', '', '', 'id', 'ASC'); 
        ...
        $asset_issues = $this->student->get_list('asset_issues' ,  array('user_id' => $student->user_id, 'check_out_date'=>NULL), '', '', '', 'id', 'ASC'); 
        if(!empty($asset_issues)){
            error($this->lang->line('this_user_have_asset_issued'));
            redirect('asset/issue/index');
        }
                
        // get library book issues
        $book_issues = $this->student->get_list('book_issues' ,  array('user_id' => $student->user_id), '', '', '', 'id', 'ASC'); 
        if(!empty($book_issues)){
            error($this->lang->line('this_student_have_book_issued'));
        ...
        if ($this->student->delete('students', array('id' => $id))) {
            ...
            $this->student->delete('library_members', array('user_id' => $student->user_id));
             ...
             // delete student book_issues            
            $this->student->delete('book_issues', array('user_id' => $student->user_id));
                       
            // delete student activity_logs
            $this->student->delete('activity_logs', array('user_id' => $student->user_id));
            
            // delete student asset_issues
            $this->student->delete('asset_issues', array('user_id' => $student->user_id));
            
            // delete student assignment_submissions
            $this->student->delete('assignment_submissions', array('student_id' => $id));
            
            // delete student awards
            $this->student->delete('awards', array('user_id' => $student->user_id));
            
            // delete student complains
            $this->student->delete('complains', array('user_id' => $student->user_id));
            ...
            $this->student->delete('results', array('student_id' => $student->id));
            
            // delete student exam_taken_exams
            $this->student->delete('exam_taken_exams', array('student_id' => $student->id));
            
            // delete student final_results
            $this->student->delete('final_results', array('student_id' => $student->id));
            
            // delete student item_issues
            $this->student->delete('item_issues', array('user_id' => $student->user_id));
            
            // delete student leave_applications
            $this->student->delete('leave_applications', array('user_id' => $student->user_id));
            
            // delete student messages
            $this->student->delete('messages', array('owner_id' => $student->user_id));
            
            // delete student message_relationships
            ...
            $this->student->delete('ratings', array('student_id' => $student->id));
            
            // delete student ss_candidates
            $this->student->delete('ss_candidates', array('student_id' => $student->id));
            
            // delete student ss_candidates
            $this->student->delete('ss_scholarships', array('student_id' => $student->id));
            
            // delete student student_activities
            $this->student->delete('student_activities', array('student_id' => $student->id));
            
            // delete student student_attendances
            $this->student->delete('student_attendances', array('student_id' => $student->id));
            
            // delete student visitors
            $this->student->delete('visitors', array('user_id' => $student->user_id));
                        
            // delete student invoices
            $this->student->delete('invoices', array('user_id' => $student->user_id));
            
            // delete student invoice_detail
            $this->student->delete('invoice_detail', array('user_id' => $student->user_id));
            
            // delete student item_sales
            $this->student->delete('item_sales', array('user_id' => $student->user_id));
            
            // delete student transactions
            $this->student->delete('transactions', array('user_id' => $student->user_id));
    ...
    * ********************************************************** */
    ...
    public function get_roll_no(){
        ...
        $roll_no = $this->db->order_by('roll_no', 'DESC')->get_where('enrollments', array('school_id'=>$school_id, 'academic_year_id'=>$school->academic_year_id, 'class_id'=>$class_id, 'section_id'=>$section_id))->row();
        
        if(!empty($roll_no)){
            echo $roll_no->roll_no+1;
        }else{
            echo 1;
        }
    }
